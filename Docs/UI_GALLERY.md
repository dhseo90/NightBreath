...
| `MetricDetailView` | metric 하나의 상세 탐색 | metric 설명, 최근 값, 단위, 기간/source filter, 그래프, 통계, raw sample list | 기간 선택, source filter, sample 확인 | HealthKit-backed/local-only 설명을 구분하고 개인 참고용으로 표시 | `ScreenshotMetricDetailScenario`, `ScreenshotLocalOnlyMetricScenario` | `Docs/Screenshots/Health/metric_detail_body_water_light.png` | screenshot pending | Release | `Docs/METRIC_DETAIL_VIEW.md` |
| `HealthCalendarView` | 월 단위 건강 캘린더 | 날짜별 수면/혈압/체성분/활동/check-in dot, sample count, data quality | 이전/다음 월, 오늘 이동, 날짜 선택 | 같은 날짜 데이터가 인과관계를 의미하지 않음을 안내 | `ScreenshotHealthCalendarScenario` | `Docs/Screenshots/Health/health_calendar_light.png` | screenshot pending | Release | `Docs/HEALTH_CALENDAR.md` |
...
| View name | 역할 | 주요 표시 데이터 | 주요 액션 | Privacy / Safety notes | Suggested scenario | Suggested screenshot path | Screenshot | Release 노출 | 관련 문서 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Report empty state | 수면 리포트 없음 | 수면 기록 후 리포트 생성 안내 | 수면 시작 | mock state로만 문서화 | 수동 empty repository state | `Docs/Screenshots/EdgeStates/report-empty.png` | screenshot pending | Release | `Docs/UI_SCREEN_MAP.md` |
...
| Fitdays import result state | 가져오기 preview/결과 | imported sample count, skipped rows, unknown columns, errors | 저장 또는 상태 확인 | synthetic fixture 또는 mock state만 사용 | `ScreenshotFitdaysImportScenario` 이후 synthetic file 수동 선택 | `Docs/Screenshots/Health/fitdays_import_result_light.png` | screenshot pending | Release | `Docs/FITDAYS_IMPORT.md` |
...
| Metric detail empty state | 특정 기간/source sample 없음 | empty 안내, 기간/source 변경 제안 | 기간 변경, source filter 변경 | 데이터 부족을 상태 해석으로 바꾸지 않음 | `ScreenshotMetricDetailScenario`에서 source filter 수동 변경 | `Docs/Screenshots/EdgeStates/metric-detail-empty.png` | screenshot pending | Release | `Docs/METRIC_DETAIL_VIEW.md` |
| Cross metric insufficient state | 비교 가능한 데이터 부족 | matched sample count, 제한 안내 | 기간/항목 변경 | 데이터가 부족하면 패턴 요약을 생성하지 않음 | `ScreenshotHealthDashboardScenario` 이후 수동 진입 | `Docs/Screenshots/EdgeStates/cross-metric-insufficient.png` | screenshot pending | Release | `Docs/CROSS_METRIC_ANALYSIS.md` |
