...
- HealthKit에 없는 Fitdays 고유 지표는 Fitdays 가져오기 화면의 수동 입력 section에서 사용자가 직접 입력한 로컬 샘플로 저장할 수 있습니다.
...
matched sample 수가 부족하면 분석 요약을 만들지 않고 제한 안내를 표시합니다. Cross Metric 화면은 부족 상태를 다음처럼 분리합니다.

- 날짜 매칭 샘플이 없음
- 날짜 매칭은 있지만 오디오 커버리지가 낮아 요약에서 제외됨
- 요약 포함 샘플 수가 기준보다 부족함

낮은 오디오 커버리지 데이터는 날짜별 매칭 목록에는 남기되, 그래프와 요약 계산에서는 제외합니다.
