...
- `HealthCalendarView` 날짜 cell은 수면, 혈압, 체성분, 활동, check-in dot을 작은 indicator로 표시합니다.
- 오늘, 선택된 날짜, 현재 월 외 날짜는 배경/테두리/opacity를 함께 사용해 구분합니다.
- 날짜 cell 안에는 긴 텍스트를 넣지 않고, 상세 데이터는 `DailyMeasurementDetailView`에서 보여줍니다.
