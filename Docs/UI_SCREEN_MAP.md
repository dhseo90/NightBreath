...
| --- | --- | --- | --- |
| `HomeDashboardView` | 앱 홈과 최근 리포트 허브 | 앱 이름, 최근 수면 리포트, 수면 소리 점수, 측정 품질, 실제 오디오 수신 시간, 녹음 커버리지, 주요 이벤트 요약, 이벤트 오디오 샘플 저장 상태, 최근 건강 기록 바로가기 | 수면 시작, 리포트 보기, 타임라인 보기, 최근 건강 기록 상세/건강 캘린더 직행, Daily Rhythm/트렌드/건강/개인정보/가이드 진입 |
...
| `HealthDashboardView` | 건강 데이터 dashboard 허브 | 예시 미리보기/read-only 연결 상태, 데이터 상태, 최근 날짜 바로가기, 로컬 import edge state, 최근 건강 지표, 데이터 출처, 전체 지표/캘린더/Fitdays import/BloodPressure/BodyComposition/CrossMetric 진입 | 건강 데이터 연결, 최근 날짜 상세 진입, 하위 dashboard 진입 |
...
README 대표 screenshot 8개는 예시 데이터와 screenshot scenario 기반으로 생성한 후보가 있지만, 2026-05-07 리뷰에서 crop 정렬과 내부 QA label 노출 문제가 확인되어 README/App Store용 렌더링을 중단했습니다. 원본 capture는 `Docs/Screenshots/README/`에 보존하되, 재캡처와 visual QA 전에는 `blocked, recapture required`로 관리합니다. 전체 화면별 설명과 pending 상태는 `Docs/UI_GALLERY.md`, 폴더 원칙은 `Docs/Screenshots/README.md`, capture 절차는 `Tools/Screenshots/README.md`를 기준으로 합니다.
...
- 홈은 최근 건강 기록 상세와 건강 캘린더로 바로 진입할 수 있지만, 이 바로가기는 HealthKit 권한 요청을 직접 시작하지 않습니다.
