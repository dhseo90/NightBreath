...
| `FitdaysImportView` | 사용자가 선택한 Fitdays CSV/export 파일 또는 월별 데이터 복사 텍스트 import 화면 | 파일 선택 상태, 붙여넣기 입력, import preview/result, 저장된 가져오기 기록, 생성 샘플 수, skipped row, unknown column, errors | 파일 선택, 월별 데이터 붙여넣기, preview 확인, import result 확인, 저장된 기록 삭제 |
...
README 대표 screenshot 8개는 예시 데이터와 screenshot scenario 기반으로 생성한 후보가 있지만, 2026-05-07 리뷰에서 crop 정렬과 내부 QA label 노출 문제가 확인되어 README/App Store용 렌더링을 중단했습니다. 원본 capture는 `Docs/Screenshots/README/`에 보존하되, 품질 gate 통과 전에는 `captured, quality review pending`으로 관리합니다. 전체 화면별 설명과 pending 상태는 `Docs/UI_GALLERY.md`, 폴더 원칙은 `Docs/Screenshots/README.md`, capture 절차는 `Tools/Screenshots/README.md`를 기준으로 합니다.
