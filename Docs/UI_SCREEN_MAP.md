...
실제 screenshot 파일이 없는 화면은 `screenshot pending`으로 관리하고 broken image link를 만들지 않습니다. 현재 직접 launch scenario가 있는 상세/edge/DEBUG-only 화면은 캡처 후보를 생성했으며 visual QA 전까지 image markdown을 만들지 않습니다.
