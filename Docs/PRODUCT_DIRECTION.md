...
하단 tab은 기능 영역을 겹치지 않게 나누는 기본 product contract입니다.

- 홈: 종합 평가입니다. 최근 수면 리포트를 중심으로 최종 상태를 점수, 주요 이벤트, 하루 리듬 카드 진입으로 보여줍니다.
- 수면: 수면 기능 동작과 수면 결과 확인입니다. 수면 시작/종료, 배치 준비, 최근 수면 리포트, 타임라인, 수면 트렌드를 담당합니다.
- 건강: Apple 건강앱 read-only 데이터, 밤숨 앱 계산 수면 지표, Fitdays 로컬 import 등 건강 데이터를 취합하는 허브입니다. 종합/개별 지표, 건강 캘린더, 날짜별 상세, Fitdays 가져오기를 이곳에 둡니다.
- 설정: 앱 설정입니다. 개인정보/저장소, 감지 민감도, iPhone 배치, 캘리브레이션, DEBUG 도구를 담당합니다.

홈에는 건강 데이터 상세 탐색을 깊게 넣지 않습니다. 건강 상세를 보고 싶을 때는 건강 tab으로 이동해 출처, 날짜, 개별 지표, import 상태를 확인합니다.
