    ...
    var body: some View {
        List {
            ...
            Section("실시간 Feature") {
                ...
                SampleCaptureRow(title: "Energy", value: viewModel.format(viewModel.currentEnergy, digits: 6))
            }
            ...
            if let features = viewModel.latestSavedFeatures {
                Section("저장된 feature summary") {
                    ...
                    SampleCaptureRow(title: "Zero Crossing", value: viewModel.format(features.zeroCrossingRate, digits: 4))
                    SampleCaptureRow(title: "Spectral Centroid", value: viewModel.format(features.spectralCentroid, digits: 1) + " Hz")
            ...
            if let savedSample = viewModel.latestSavedSample {
                Section("저장 결과") {
                    ...
                    SampleCaptureRow(title: "Feature CSV", value: savedSample.featureCSVURL.lastPathComponent)
            ...
            Section("저장 안전장치") {
                SampleCaptureRow(title: "Debug sample count", value: "\(viewModel.sampleStorageStatus.sampleCount)개")
                SampleCaptureRow(title: "Debug sample folder size", value: viewModel.formatBytes(viewModel.sampleStorageStatus.folderSizeBytes))
                ...
                SampleCaptureRow(title: "샘플 1개 최대 길이", value: "\(Int(viewModel.storagePolicy.maxSampleDuration))초")
                SampleCaptureRow(title: "앱 실행당 최대 샘플", value: "\(viewModel.storagePolicy.maxSamplesPerSession)개")
                SampleCaptureRow(title: "샘플 폴더 최대 용량", value: viewModel.formatBytes(viewModel.storagePolicy.maxFolderSizeBytes))
                SampleCaptureRow(title: "오래된 샘플 정리", value: "7일")
                SampleCaptureRow(title: "Raw full-night audio storage", value: viewModel.fullNightStorageStatusText)
            }
