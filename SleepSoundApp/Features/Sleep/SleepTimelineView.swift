...
private struct EventRow: View {
    ...
    var body: some View {
        ...
        .background(Color(.secondarySystemBackground))
        .clipShape(RoundedRectangle(cornerRadius: 8))
        .confirmationDialog("이 이벤트 오디오 샘플을 삭제할까요?", isPresented: $showDeleteSnippetConfirmation, titleVisibility: .visible) {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 8) {
                ForEach(SleepEventFeedbackSelection.allCases) { selection in
                    ...
                    .tint(feedback?.selectedFeedback == selection ? .orange : .secondary)
                }
            }
        }
    }
...
private final class SleepEventFeedbackViewModel: ObservableObject {
    ...
    func hasSnippet(for event: SleepEvent) -> Bool {
        guard let fileName = event.audioSnippetFileName,
              !deletedSnippetFileNames.contains(fileName) else {
            return false
        }
    ...
    func playSnippet(for event: SleepEvent) {
        guard hasSnippet(for: event),
              let fileName = event.audioSnippetFileName else {
            return
        }
