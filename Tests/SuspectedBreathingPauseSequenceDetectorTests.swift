...
struct SuspectedBreathingPauseSequenceDetectorTests {
    ...
    @Test
    func sustainedLowActivityCreatesCandidate() throws {
        let detector = SuspectedBreathingPauseSequenceDetector(minimumLowActivityDuration: 10)
        let result = detector.detect(
            features: lowActivityFeatures(duration: 10),
            contextOutputs: []
        )
        let output = try #require(result.outputs.first)

        #expect(output.eventType == .breathingPauseSuspected)
        #expect(output.duration == 10)
        #expect(output.confidence >= 0.30)
    ...
    func gaspLikeRecoveryRaisesConfidence() throws {
        let detector = SuspectedBreathingPauseSequenceDetector(minimumLowActivityDuration: 10)
        let features = lowActivityFeatures(duration: 12)
        ...
        let gaspOutput = makeOutput(
            .gaspLike,
            start: baseDate.addingTimeInterval(12.5),
            duration: 1,
            confidence: 0.70
        )
        let withRecoveryResult = detector.detect(
            features: features,
            contextOutputs: [gaspOutput]
        )
    ...
    func environmentalNoiseLowersCandidateConfidence() throws {
        ...
        let noisyResult = detector.detect(
            features: lowActivityFeatures(duration: 12, estimatedNoiseLevel: 0.24),
            contextOutputs: [
        ...
        #expect(noisyResult.summary.noiseContaminatedLowActivityCount == 1)
    }
    ...
    private func lowActivityFeatures(
        ...
        (0..<duration).map { offset in
            AudioFeatures(
                startedAt: baseDate.addingTimeInterval(TimeInterval(offset)),
                duration: 1,
                rms: 0.002,
                peak: 0.004,
